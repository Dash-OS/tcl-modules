...
- A local variable `$opts` is provided.  A `dict` with the options that were called.  Single element options (-opt) have a boolean value of 1, two element options have a value set to the value given by the caller.
...
### Implementation
  ...
  - A pure-tcl initial implementation may be found as [optcmds](https://github.com/Dash-OS/tcl-modules/blob/master/optcmds-1.0.0.tm) which provides `oproc`, `oapply`, and `omethod`.
...
### What about upvar, defaults, switch, and/or required?
...
With that in mind, these "extras" become noise that make the syntax more difficult to learn, read, and understand quickly.  

Without these extra options we are able to provide a syntax that quite literally speaks for itself in more ways than one.

In addition, there is likely to be many circumstances even if these are provided that the
programmed would just set them as optional anyway so custom/better error messaging or handling could be provided.
...
> This could be further sugared if something like `[dict haskeys $opts -upvar -command]` were available, giving a general purpose solution on top of this.
