...
### Implementation
  ...
  - A pure-tcl initial implementation may be found as [optcmds](https://github.com/Dash-OS/tcl-modules/blob/master/optcmds-1.0.4.tm) which provides `oproc`, `oapply`, and `omethod`.
   ...
   - > In addition, in this implementation one can change the name of the opts var by providing an option to the oproc/apply/method commands -opts with the desired name of the variable.  `-opts myOptsDict`
   - > For those that are against the creation of an `opts dict`, it is opt-out by either giving the opts an empty name (`-opts {}`) or providing the `-noopts` flag to the optcmd.
