...
### Implementation
  ...
  - A pure-tcl initial implementation may be found as [optcmds](https://github.com/Dash-OS/tcl-modules/blob/master/optcmds-1.0.2.tm) which provides `oproc`, `oapply`, and `omethod`.
...
There is some demand for default values being available for any kind of option/argument handling improvements.  Since `$opts` is a `dict`,  this is trivial to implement in the body with minimal (if any) performance difference.  A cleaner syntax within the options definitions is preferred by this proposal at this time.  

```tcl
proc somecall {-command callback -timeout ms -url url -method method --} {
  set opts [dict merge {-timeout 15000 -method GET} $opts]
...
> This could be further sugared if something like `[dict haskeys $opts -upvar -command]` were available, giving a general purpose solution on top of this which can be used in many other places as well.
