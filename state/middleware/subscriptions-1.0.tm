...
module create ::state::middleware::subscriptions {
	...
	method finished { keyValue {evalID {}} } {
		if { $evalID eq {} } {
	...
	method cancelEvaluation { keyValue evalID } {
		if { 
				 $evalID ne "async"
		...
		if { [dict exists $EVALUATIONS $keyValue $evalID asyncID] } {
			after cancel [dict get $EVALUATIONS $keyValue $evalID asyncID]
		}
		rename [dict get $EVALUATIONS $keyValue $evalID evaluator] {}
	...
	method onSnapshot { snapshot } {
		try {	
			...
			if { $hasAsync } {
				...
				dict set evalSchema asyncID $asyncID
				if { $BATCH } {
	...
	method BuildEvaluator { evalID snapshot } {
	...
	# next evaluation.
	method batch { snapshot keyValue } {
	...
	method inject { evaluator script } {
	...
	method filter { type snapshot keyValue evalID } {
		...
		dict for { priority subs } [dict get $SUBSCRIPTIONS $type] {
	    	...
	    	foreach active $actives {
	    		if { $active ne "*" && $active ne {} && ! [ lhas $active [dict get? $snapshot set] ] } {
	...
	# ::oo::define ::tcm::module::state-subscription-middleware::SubscriptionMiddleware
	method search { type subscriptions snapshot keyValue evalID } {
		if { $type eq "async" } { 
		...
		dict for {subscriptionID subscriptionSchema} $subscriptions {
			...
			if { [info exists passed] } {
				...
				my inject $evaluator {
					...
					try [dict get $subscriptionSchema script]
		...
		# puts "[self method] PASS $subscriptionID : [expr { [clock microseconds] - $::start }] microseconds"
		if { $type eq "async" } { my finished $keyValue $evalID } elseif { 
    	! [dict exists $EVALUATIONS $keyValue $evalID asyncID] 
			&& ! [dict exists $EVALUATIONS $keyValue async]
    } { my finished $keyValue $evalID }
	}
	
	# method Check { args } {
	# 	puts "Cleanup Check $args"
	# 	puts $EVALUATIONS
	# }
	# ::oo::define ::tcm::module::state-subscription-middleware::SubscriptionMiddleware
	method evaluateRules { or subscription snapshot keyValue evalID } {
		set result 0
		foreach rule $or {
