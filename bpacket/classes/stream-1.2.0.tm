...
::oo::define ::bpacket::classes::stream {
  variable STATUS PACKETS CALLBACK
...
::oo::define ::bpacket::classes::stream method event callback {
  ...
  if {[dict size $PACKETS] && ![dict exists $TIMEOUTS dispatch]} {
...
::oo::define ::bpacket::classes::stream method flush {{bufferID default}} {
  ...
  while {[set buffer [bpacket headerstart $buffer]] ne {}} {
    ...
    if {![dict exists $BUFFER $bufferID size] || ![dict exists $BUFFER $bufferID start]} {
      lassign [bpacket decode varint $buffer] size _ start
      # _ is the "remaining data" after parsing the varint - in this
      # case we do not care about that
      if {![string is entier -strict $size] || ![string is entier -strict $start]} {
        # if we can not get a valid size/start value then we have to trash this
        # packet and move on to the next
        set buffer [bpacket nextheader]
        if {$buffer eq {}} {
          break
        }
    ...
    if {$packet_length == $size && [string match "*${::bpacket::EOF}" $buffer]} {
      ...
      if {[string match "*${::bpacket::EOF}" $packet]} {
        ...
        set buffer [bpacket nextheader $packet$buffer]
      }
    } else {
      # We have received a partial packet.  In this case
      # we simply have to wait for more data to be appended.
      # -- we dont want the standard method as it will remove
      #    the packet size and start which we want to utilize
      #    on all future additions to the stream.
      break
    }
  ...
  # if we have any remaining buffer then we save it for later
  if {$buffer ne {}} {
    dict set BUFFER $bufferID buffer $buffer
  } elseif {[dict exists $BUFFER $bufferID]} {
...
# $stream append $pkt
# DEPRECIATED?  Simply receive packets - no need to handle
